...
def objective(trial, cfg: DictConfig, trial_configs, trial_stats, trial_importances):

    config = copy.deepcopy(cfg) # create config for training upstream model with suggested parameters
    config.mode = 'feature_selection'

    model_params, training_params = get_parameters(cfg.model.name, trial, cfg.hyp, mode='_upstream')
    ...
    model_params_dwnst, training_params_dwnst = get_parameters(cfg.model_downstream.name, trial, cfg.hyp_downstream, mode='_downstream')
    ...
    trial_importances.append(importances.numpy())
    return stats['val_stats']['score']

@hydra.main(config_path="config", config_name="tune_full_pipeline_config")
def main(cfg):
    optuna_seed=10
    n_optuna_trials = 100
    ...
    func = lambda trial: objective(trial, cfg, trial_configs, trial_stats, trial_importances)
    ...
    np.savetxt('all_importances.out', np.vstack(trial_importances), delimiter=',')
